...
class LaikagoEnv(gym.Env):
	...
	def hard_reset(self):
		...
		model_path = 'gym_stoch2_sloped_terrain/envs/robots/stoch_4_urdf/urdf/stoch_4_urdf.urdf'
	...
	def transform_action(self, action):
		...
		action[:4] = action[:4] * 3 * 0.068  # Max steplength = 3x0.068
	...
	def do_simulation(self, action, n_frames):
		...
		all_joint_angles = self._pybullet_client.calculateInverseKinematics2(self.Laikago, [2,5,8,11], world_ee_pts, solver=0)
	...
	def ResetLeg(self):
		...
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["FLKM_Joint"],  # motor
			targetValue=0.67 - 0.6, targetVelocity=0)

		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["FLKM_Joint"]),
		...
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["FRKM_Joint"],
			targetValue=0.67 - 0.6, targetVelocity=0)

		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["FRKM_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		...
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["BLKM_Joint"],  # motor
			targetValue=0.67 - 0.6, targetVelocity=0)

		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["BLKM_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		...
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["BRKM_Joint"],
			targetValue=0.67 - 0.6, targetVelocity=0)

		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["BRKM_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		...
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["FLK_Joint"],  # motor
			targetValue=-1.25 + 0.66, targetVelocity=0)

		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["FLK_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		...
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["FRK_Joint"],
			targetValue=-1.25 + 0.66, targetVelocity=0)

		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["FRK_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		...
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["BLK_Joint"],  # motor
			targetValue=-1.25 + 0.66, targetVelocity=0)

		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["BLK_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		...
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["BRK_Joint"],
			targetValue=-1.25 + 0.66, targetVelocity=0)

		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["BRK_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
	...
	def ResetPoseForAbd(self):
		...
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["FL_Joint"],
			targetValue=0, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["FR_Joint"],
			targetValue=0, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["BL_Joint"],
			targetValue=0, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.Laikago,
			self._joint_name_to_id["BR_Joint"],
			targetValue=0, targetVelocity=0)

		# Set control mode for each motor and initial conditions
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["FL_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["FR_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["BL_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.Laikago,
			jointIndex=(self._joint_name_to_id["BR_Joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
