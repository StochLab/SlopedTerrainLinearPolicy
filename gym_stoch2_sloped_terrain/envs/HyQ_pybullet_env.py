...
class HyQEnv(gym.Env):
	...
	def hard_reset(self):
		...
		model_path = 'gym_stoch2_sloped_terrain/envs/robots/hyq/hyq.urdf'
		...
		if self._on_rack:
			self._pybullet_client.createConstraint(
				self.HyQ, -1, -1, -1, self._pybullet_client.JOINT_FIXED,
				[0, 0, 0], [0, 0, 0], [0, 0, 0.3])

		self._pybullet_client.resetBasePositionAndOrientation(self.HyQ, self.INIT_POSITION, self.INIT_ORIENTATION)
		self._pybullet_client.resetBaseVelocity(self.HyQ, [0, 0, 0], [0, 0, 0])
	...
	def apply_Ext_Force(self, x_f, y_f,link_index= 1,visulaize = False,life_time=0.01):
		...
		self._pybullet_client.applyExternalForce(self.HyQ, link_index, forceObj=[x_f,y_f,0],posObj=[0,0,0],flags=self._pybullet_client.LINK_FRAME)
		f_mag = np.linalg.norm(np.array(force_applied))

		if(visulaize and f_mag != 0.0):
			point_of_force = self._pybullet_client.getLinkState(self.HyQ, link_index)[0]
	...
	def SetLinkMass(self,link_idx,mass=0):
		...
		'''
		link_mass = self._pybullet_client.getDynamicsInfo(self.HyQ,link_idx)[0]
		if(link_idx==0):
			link_mass = mass+1.1
			self._pybullet_client.changeDynamics(self.HyQ, 0, mass=link_mass)
		elif(link_idx==11):
			link_mass = mass+1.1
			self._pybullet_client.changeDynamics(self.HyQ, 11, mass=link_mass)
	...
	def getlinkmass(self,link_idx):
		...
		'''
		m = self._pybullet_client.getDynamicsInfo(self.HyQ,link_idx)
		return m[0]
	...
	def get_foot_contacts(self):
		...
		for leg in range(4):
			contact_points_with_ground = self._pybullet_client.getContactPoints(self.plane, self.HyQ, -1, foot_ids[leg])
			if len(contact_points_with_ground) > 0:
				foot_contact_info[leg] = 1

			if self._is_wedge:
				contact_points_with_wedge = self._pybullet_client.getContactPoints(self.wedge, self.HyQ, -1, foot_ids[leg])
				if len(contact_points_with_wedge) > 0:
					foot_contact_info[leg+4] = 1

			if self._is_stairs:
				for steps in self.stairs:
					contact_points_with_stairs = self._pybullet_client.getContactPoints(steps, self.HyQ, -1,
																					   foot_ids[leg])
	...
	def do_simulation(self, action, n_frames):
		...
		plane_normal,self.support_plane_estimated_roll,self.support_plane_estimated_pitch = normal_estimator.vector_method_Hyq(self.prev_incline_vec, contact_info, self.GetMotorAngles(), Rot_Mat)
	...
	def GetMotorAngles(self):
		...
		'''
		motor_ang = [self._pybullet_client.getJointState(self.HyQ, motor_id)[0] for motor_id in self._motor_id_list]
		return motor_ang
	...
	def GetMotorVelocities(self):
		...
		'''
		motor_vel = [self._pybullet_client.getJointState(self.HyQ, motor_id)[1] for motor_id in self._motor_id_list]
		return motor_vel
	...
	def GetBasePosAndOrientation(self):
		...
		'''
		position, orientation = (self._pybullet_client.getBasePositionAndOrientation(self.HyQ))
	...
	def GetBaseAngularVelocity(self):
		...
		'''
		basevelocity= self._pybullet_client.getBaseVelocity(self.HyQ)
	...
	def SetFootFriction(self, foot_friction):
		...
		'''
		FOOT_LINK_ID = [7,3,15,11]
		for link_id in FOOT_LINK_ID:
			self._pybullet_client.changeDynamics(
			self.HyQ, link_id, lateralFriction=foot_friction)
	...
	def BuildMotorIdList(self):
		...
		'''
		num_joints = self._pybullet_client.getNumJoints(self.HyQ)
		joint_name_to_id = {}
		for i in range(num_joints):
			joint_info = self._pybullet_client.getJointInfo(self.HyQ, i)
			joint_name_to_id[joint_info[1].decode("UTF-8")] = joint_info[0]

			# adding abduction
			MOTOR_NAMES = ["lf_hfe_joint",
						   "lf_kfe_joint",
						   "rf_hfe_joint",
						   "rf_kfe_joint",
						   "lh_hfe_joint",
	...
	def ResetLeg(self):
		...
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["lf_hfe_joint"],  # motor
			targetValue=0.7, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["lf_kfe_joint"],
			targetValue=-1.4, targetVelocity=0)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["lf_hfe_joint"]),
		...
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["lf_kfe_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		...
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["rf_hfe_joint"],  # motor
			targetValue=0.7, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["rf_kfe_joint"],
			targetValue=-1.4, targetVelocity=0)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["rf_hfe_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["rf_kfe_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		...
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["lh_hfe_joint"],  # motor
			targetValue=-0.7, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["lh_kfe_joint"],
			targetValue=1.4, targetVelocity=0)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["lh_hfe_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["lh_kfe_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		...
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["rh_hfe_joint"],  # motor
			targetValue=-0.7, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["rh_kfe_joint"],
			targetValue=1.4, targetVelocity=0)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["rh_hfe_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["rh_kfe_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
	...
	def ResetPoseForAbd(self):
		...
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["lf_haa_joint"],
			targetValue=0, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["rf_haa_joint"],
			targetValue=0, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["lh_haa_joint"],
			targetValue=0, targetVelocity=0)
		self._pybullet_client.resetJointState(
			self.HyQ,
			self._joint_name_to_id["rh_haa_joint"],
			targetValue=0, targetVelocity=0)

		# Set control mode for each motor and initial conditions
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["lf_haa_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["rf_haa_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["lh_haa_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
		self._pybullet_client.setJointMotorControl2(
			bodyIndex=self.HyQ,
			jointIndex=(self._joint_name_to_id["rh_haa_joint"]),
			controlMode=self._pybullet_client.VELOCITY_CONTROL,
			force=0,
			targetVelocity=0
		)
